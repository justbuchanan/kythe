...
class TextprotoAnalyzer {
 public:
  ...
  Status AnalyzeAnyTypeUrl(const proto::VName& file_vname,
...
// regex to find the "mypackage.MyMessage" portion and add an anchor node and a
// ref to the proto message descriptor. Ideally this information would be
// provided in the ParseInfoTree generated by the textproto parser, but since
// it's not, we do our own "parsing" with a regex.
Status TextprotoAnalyzer::AnalyzeAnyTypeUrl(
  ...
  if (!re2::RE2::PartialMatch(sp, R"(\[\s*[^/]+/([^\s\]]+)\s*\])", &match)) {
    LOG(ERROR) << "Unable to find type_url span for Any";
    return OkStatus();
  }
